...
// read file
try {
  var data = fs.readFileSync('lod2.eu.json', 'ascii');
...
var hasSubject = dstore.hasSubject("http://seebi.myopenid.com/", function(err, status) {
  var stop = process.hrtime();
	console.log('HasSubject took ' + (stop[1] - start[1])/1000000 + ' ms!');
	console.log("\nhttp://seebi.myopenid.com/ is already inserted:  " + status + '\n');
});

// find
var start = process.hrtime();
var find = dstore.find("http://seebi.myopenid.com/", function(err, result) {
  var stop = process.hrtime();
